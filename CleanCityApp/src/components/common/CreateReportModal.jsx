...
export default function CreateReportModal({ onClose, onSuccess }) {
  ...
  // GPS toggle: point-in-polygon first, Nominatim as fallback
      ...
      async (pos) => {
        ...
        try {
          ...
          if (fallback) {
            ...
            setGpsError(`Không tìm thấy phường/xã cho "${wardCandidate || 'vị trí này'}". Vui lòng chọn thủ công.`)
          }
        } catch {
  ...
  return (
    <div className="modal-overlay bg-on-surface/20 backdrop-blur-md">
      <div className="bg-surface-container-lowest w-full max-w-4xl max-h-[90vh] overflow-y-auto custom-scrollbar rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden border border-outline-variant/10">
        {/* ── LEFT: Media & Map ────────────────────────── */}
        <div className="w-full md:w-2/5 bg-surface-container p-4 sm:p-6 space-y-4 sm:space-y-6">
          {/* Image upload */}
          <div>
            ...
            {imagePreview ? (
              <div className="relative aspect-video sm:aspect-square w-full rounded-xl overflow-hidden">
              ...
              <div
                ...
                className="aspect-video sm:aspect-square w-full rounded-xl bg-surface-container-highest border-2 border-dashed border-outline-variant flex flex-col items-center justify-center text-center p-4 group cursor-pointer hover:border-primary transition-all"
              >
                ...
                <div className="absolute top-2 left-2 z-[1000] bg-surface/90 backdrop-blur px-2 py-1 rounded-lg">
                  <span className="text-[10px] font-bold text-primary">
                    {boundaryLoading
                      ? 'Đang tải ranh giới...'
                      : gpsEnabled && pin
                      ? 'Vị trí GPS · Nhấn bản đồ để điều chỉnh'
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  title={gpsEnabled ? 'Tắt dùng vị trí hiện tại' : 'Dùng vị trí hiện tại'}
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold transition-all border ${
                    gpsLoading
                      ? 'border-outline-variant text-outline cursor-wait'
                      : gpsEnabled
                      ? 'bg-primary text-on-primary border-primary shadow-sm'
                      : 'border-outline-variant text-outline-variant hover:border-primary hover:text-primary'
                  }`}
                >
                  <span className="material-symbols-outlined text-sm" style={{ fontVariationSettings: gpsEnabled ? "'FILL' 1" : "'FILL' 0" }}>
                    {gpsLoading ? 'sync' : 'my_location'}
                  </span>
                  {gpsLoading ? 'Đang định vị...' : gpsEnabled ? 'GPS bật' : 'Vị trí GPS'}
                </button>
              </div>
              <select
                value={wardId}
              ...
              {gpsError && <p className="text-xs text-error font-medium">{gpsError}</p>}
              {!gpsError && wardsLoading && <p className="text-xs text-on-surface-variant">Đang tải phường/xã...</p>}
              {gpsEnabled && !gpsError && wardId && (
                <p className="text-xs text-primary font-medium flex items-center gap-1">
                  <span className="material-symbols-outlined text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
                  Đã chọn phường/xã từ vị trí GPS
                </p>
              )}
            </div>
