...
const ROLE_OPTIONS = ["Tất cả", "Admin", "User", "Collector"];
...
export default function AccountsPage() {
	...
	const handleDelete = async (id) => {
		try {
			...
			swalError('Xoá tài khoản thất bại.');
		}
	};
			...
			<section className="bg-surface-container-low rounded-[2rem] p-6 space-y-6">
					...
					{loading ? (
						...
						<table className="w-full text-left border-collapse bg-surface-container-lowest">
							...
							<tbody className="divide-y divide-outline-variant/10">
								{paged.map(acc => (
									<tr key={acc.id} className="hover:bg-surface-container-low transition-colors group">
										...
										<td className="px-6 py-5 text-right">
											<div className="flex justify-end gap-2 opacity-40 group-hover:opacity-100 transition-opacity">
												...
												<button className={`p-1.5 ${acc.status === 'Active' ? 'hover:text-error' : 'hover:text-primary'} transition-colors`} title="Khoá/Mở" onClick={() => handleDelete(acc.id)}>
				...
				<div className="flex items-center justify-between pt-4">
					<p className="text-xs font-bold text-on-surface-variant">
						Hiển thị <span className="text-primary">{(page-1)*pageSize+1}-{Math.min(page*pageSize, total)}</span> trên tổng số <span className="text-primary">{total}</span> người dùng
					</p>
					<div className="flex items-center gap-1">
						<button className="p-2 rounded-lg hover:bg-surface-container-highest transition-colors disabled:opacity-30" disabled={page === 1} onClick={() => setPage(page-1)}>
							<span className="material-symbols-outlined">chevron_left</span>
						</button>
						{Array.from({length: totalPages}, (_, i) => i+1).slice(Math.max(0, page-2), page+2).map(p => (
							<button
								key={p}
								className={`w-10 h-10 rounded-lg ${p===page ? 'bg-primary text-on-primary font-black' : 'hover:bg-surface-container-highest font-bold'} text-sm`}
								onClick={() => setPage(p)}
							>{p}</button>
						))}
						{totalPages > 5 && page < totalPages - 2 && <span className="px-2">...</span>}
						{totalPages > 5 && page < totalPages - 2 && (
							<button className="w-10 h-10 rounded-lg hover:bg-surface-container-highest font-bold text-sm" onClick={() => setPage(totalPages)}>{totalPages}</button>
						)}
						<button className="p-2 rounded-lg hover:bg-surface-container-highest transition-colors disabled:opacity-30" disabled={page === totalPages || totalPages === 0} onClick={() => setPage(page+1)}>
