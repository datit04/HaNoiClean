...
export default function ReportManager() {
  ...
  const [statsAll, setStatsAll] = useState({ total: 0, inProgress: 0, completed: 0, rejected: 0, currentMonth: 0, previousMonth: 0 })
  ...
  useEffect(() => {
    ...
    getReportStats()
      ...
      .catch(() => setStatsAll({ total: 0, inProgress: 0, completed: 0, rejected: 0, currentMonth: 0, previousMonth: 0 }))
  }, [])
        ...
        <div className="bg-surface-container-highest p-6 rounded-3xl relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <div className="mt-4 flex items-center gap-1 text-xs font-bold text-primary">
              ...
              {(() => {
                const { currentMonth = 0, previousMonth = 0 } = statsAll;
                let percent = 0;
                if (previousMonth > 0) {
                  percent = Math.round(((currentMonth - previousMonth) / previousMonth) * 100);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-outline-variant bg-surface-container">
                <th className="px-5 py-4 text-left font-semibold text-on-surface-variant">ID</th>
            ...
            <tbody>
              {loading ? (
                ...
                reports.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-outline-variant/50 hover:bg-surface-container/50 transition-colors">
                      <td className="px-5 py-4 font-mono text-xs text-on-surface-variant">#{r.id}</td>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-4 border-t border-outline-variant">
            <span className="text-xs text-on-surface-variant">
              Trang {page} / {totalPages} &middot; {totalRecords} báo cáo
            </span>
            <div className="flex items-center gap-1">
              <button
                disabled={page <= 1}
                onClick={() => setPage((p) => p - 1)}
                className="p-2 rounded-xl hover:bg-surface-container disabled:opacity-40 transition-colors"
              >
                ...
                .map((p, idx, arr) => {
                  ...
                  return (
                    <span key={p} className="contents">
                      {showEllipsis && <span className="px-1 text-on-surface-variant">…</span>}
                      <button
                        onClick={() => setPage(p)}
