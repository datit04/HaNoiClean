...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    getCategories({ skipAuthRedirect: true })
      .then((res) => {
  ...
  const markers = useMemo(() => {
    ...
    // Không chọn danh mục nào → ẩn hết marker
    if (filters.categoryIds.length === 0) return []
    // Chọn tất cả → hiện tất cả (tối ưu, bỏ qua filter)
    if (filters.categoryIds.length === categories.length) return visibleMarkers
    return visibleMarkers.filter((m) => {
      const catId = m.categoryId ?? m.category?.id
      return catId != null && filters.categoryIds.includes(catId)
    })
  ...
  const handleNewReport = () => {
    if (!isAuthenticated) {
      navigate(ROUTES.LOGIN, { state: { from: '/ban-do' } })
      return
    }
