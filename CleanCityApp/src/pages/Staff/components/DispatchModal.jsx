...
export default function DispatchModal({ report, onClose, onSuccess }) {
  ...
  // Determine current report status to show appropriate next steps matching BE flow
  const currentStatus = report?.status ?? 0
  const availableSteps = (() => {
    switch (currentStatus) {
      case ReportStatus.Submitted:
        return STATUS_STEPS.filter((s) => s.value === ReportStatus.Received || s.value === ReportStatus.Rejected)
      case ReportStatus.Received:
        return STATUS_STEPS.filter((s) => s.value === ReportStatus.InProgress || s.value === ReportStatus.Rejected)
      case ReportStatus.InProgress:
        return STATUS_STEPS.filter((s) => s.value === ReportStatus.Completed || s.value === ReportStatus.Rejected)
      default:
        return []
    }
  })()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedStatus === ReportStatus.InProgress) {
        // BE assign endpoint sets status to InProgress automatically
        if (!selectedTeamId) {
          swalError('Vui lòng chọn đội xử lý')
          setSubmitting(false)
          return
        }
      ...
      swalSuccess('Điều phối thành công!')
      onSuccess?.()
      onClose()
    } catch (err) {
